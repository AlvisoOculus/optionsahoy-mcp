...
function extractorPrompt(conversation: string): string {
  ...
  return [
    ...
    '- Include EVERY field the user provides, not just the required ones. Map their words to the field names: "17% growth" -> expectedGrowth: 0.17, "0.72 vol" / "volatility 0.72" -> volatility: 0.72, "married" -> filingStatus: "married_joint", "$300k income" -> ordinaryIncome: 300000, percentages as decimals. When the user names a stock, in ANY phrasing ("my NVDA position", "I hold AAPL", "5,000 NVDA shares", "ticker MSFT", "ticker: MSFT", "symbol MSFT", "the ticker is MSFT", "for MSFT"), set ticker to that symbol (e.g. "NVDA"). An explicit "ticker X" is the strongest possible signal; never re-ask for a ticker the user has already named (a real symbol only: "unknown", "none", "private" and "TBD" are NOT tickers, so omit ticker and follow the growth rule below).',
    ...
    '- industry (qsbs_check only) maps the corporation\'s line of business to the closest allowed value, using the exact token: software, SaaS, internet or "tech" -> "tech-software"; hardware or manufacturing -> "manufacturing"; biotech, pharma, life sciences or drug development -> "biotech-research"; retail, wholesale or e-commerce -> "retail-wholesale"; a hotel, motel, restaurant, bar or cafe -> "hospitality"; a farm, ranch, orchard or timber operation -> "farming"; oil, gas, mining, minerals or any business that extracts a raw material -> "extraction"; law -> "law"; accounting or actuarial work -> "accounting-actuarial"; health services or a medical practice -> "health-services"; engineering -> "engineering"; architecture -> "architecture"; consulting, advisory, or market or contract research -> "consulting"; banking, insurance, brokerage or investing -> "finance"; performing arts or athletics -> "performing-arts"; a named business fitting none of these -> "other-services"; nothing said about the line of business -> "unsure". Only the first four qualify under Section 1202: hospitality, farming and extraction are separately EXCLUDED, so never round a restaurant, farm, hotel or mining business up into a qualifying bucket, and when the user describes the work as services, consulting or professional advice, use that service value even if they also call the company "tech". Do not clarify for industry when the user has named a line of business.',
    '',
...
async function handleQuery(ctx: PagesContext, req: PoeRequest, extractor?: Extractor): Promise<Response> {
  ...
  // START the market-data warm here, BEFORE the billing round-trip, and await
  // it just before the handler. Poe is the one surface with a mandatory network
  // hop (poeCost 'authorize') between knowing the tool and running it, so on
  // this path the fetches are free: they overlap a wait we were already paying.
  // Not awaited here on purpose - neither warm ever rejects or throws (see
  // lib/data/live-vols and lib/data/live-chain), so a floating handle cannot
  // produce an unhandled rejection even on the paths that return before the
  // await.
  //
  ...
  const volGateArgs =
    extracted.args !== null && typeof extracted.args === 'object' && !Array.isArray(extracted.args)
      ...
      : {};
  const volWarm = warmForCall(tool.name, volGateArgs);
