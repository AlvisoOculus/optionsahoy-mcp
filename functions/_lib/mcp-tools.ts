...
// One sentence, appended to every tool description: the free interactive
// version of the same calculator. tools/list is the single most-read surface
// (every client reads all descriptions at connect), and until now no
// description mentioned the web tools at all. Kept to one sentence: registry
// scorers rate description quality, and the TDQS rule is revert-on-regression.
const freeToolNote = (slug: string, tool: string): string =>
  ` A free interactive version of this calculator, with charts, is at https://optionsahoy.com/tools/${slug}?src=mcp_${tool} (no signup).`;
