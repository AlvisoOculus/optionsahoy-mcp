...
export const TOOLS: McpTool[] = [
  {
    ...
    description:
      'Multi-year Incentive Stock Option (ISO) exercise schedule that minimizes federal and state Alternative Minimum Tax (AMT). Use this tool for ISO planning where AMT is the binding constraint; for NSO grants use `nso_calculate`, for RSUs at vest use `rsu_sell_vs_hold`, for §1202 QSBS qualification use `qsbs_check`. Models AMT credit recovery across future years, grant-expiration timing, and the post-termination exercise window. Pure deterministic computation: no network access, no PII retention; federal + 50-state tax tables and AMT brackets are compiled into the function bundle. The optimizer searches the full feasible share-per-year space (exhaustive, not heuristic). Returns a top-level object with keys: `schedules` (object containing `lumpSum`, `evenSplit`, and `optimized` — each {nfv, federalLTCG, stateLTCG, amtPremiumFV, grossGain}), `crossoverShares` (max shares that can be exercised in year 1 before tentative AMT exceeds regular tax), `crossoverBargain`, `alreadyInAmt` (boolean), `timing` (grant expiration / qualifying disposition / 90-day window flags), `stateHasAmt`, `bargainPerShare`, `effectiveHorizon`, and `departedRecommendation` when applicable.' + STRICT_INPUT_NOTE,
    inputSchema: {
      type: 'object',
      required: [
  ...
  },
  {
    ...
    description:
      'After-tax payout on a non-qualified stock option (NSO) exercise: federal, state, FICA (Social Security + Medicare + Additional Medicare). Use this tool for non-qualified options; for incentive options needing AMT optimization use `amt_iso_optimize`, for RSUs at vest use `rsu_sell_vs_hold`. Compares sell-at-exercise vs hold-for-long-term-capital-gains across the chosen horizon. Pure deterministic computation: no network access; tax tables compiled in. The optional ticker shortcut reads a bundled snapshot of the trailing-CAGR table (refreshed daily upstream). Returns a top-level object with keys: `exercise` (tax breakdown at exercise: bargainElement, federal, state, socialSecurity, medicare, additionalMedicare, total, netCashSellAll, sharesSoldToCover, sharesRetained), `hold` (LTCG outcome at end of holdYears: expectedGain, capGainFederal, capGainState, capGainTotal, isLongTerm), `sellNowInvest` (counterfactual: sell at exercise and reinvest at expectedMarketReturn), `holdMinusCashless` (the dollar delta between hold and sell-now-invest), and `bracketJump` (fromRate, toRate, thresholdAtJump describing the marginal bracket change at exercise).' + STRICT_INPUT_NOTE,
    inputSchema: {
      type: 'object',
      required: [
  ...
  },
  {
    ...
    description:
      'Single-stock concentration risk analysis. Use this tool to compare strategies on an existing concentrated single-stock position. For standalone hedge pricing without tax context use `protective_put_price`; for the tax math on the option exercise or RSU vest that created the concentration, use the matching `amt_iso_optimize` / `nso_calculate` / `rsu_sell_vs_hold` tool first. Quantifies drawdown exposure at 30%, 50%, and 70% downside scenarios. Compares three after-tax strategies across a three-year horizon: sell down to a target weight, hold and accept volatility, or hedge with a protective put or zero-cost collar. Computes federal long-term capital gains tax, state tax, the 3.8% Net Investment Income Tax (NIIT), and the reinvestment opportunity cost. Reports per-strategy net final value (NFV). `totalAssets` is the user\'s total investable portfolio (the concentrated position plus everything else); the analysis frames risk relative to it and MUST come from the user, never inferred. Returns a top-level object with keys: `concentration` (position/totalAssets as a decimal), `riskBand` (Low / Moderate / Concentrated / Highly concentrated / Extreme), `isLongTermToday` (boolean), `longTermDate`, `daysUntilLongTerm`, `lossExposure` (array of {drop, dollarLoss, newConcentration} for 30/50/70% drops), `waitForLtInsight`, `schedule` (array of yearly sales with per-year federalTax/stateTax/totalTax), `hedging` (present when hedgeChoice provided: hedged-strategy NFV + cost), `sectorContextLine`, and `advisorBenchmarkLine`.' + STRICT_INPUT_NOTE,
    inputSchema: {
      type: 'object',
      required: [
  ...
  },
  {
    ...
    description:
      'Black-Scholes pricing of a protective put or zero-cost collar on a single-stock position. Use this tool for standalone hedge pricing on any single-stock position (no tax math). For concentration-vs-hedge comparison with multi-year tax cost, use `concentration_analyze` with a `hedgeChoice` instead. Reports annualized hedge cost as a percentage of position value, maximum loss with the hedge in place, upside participation cap (collar only, where the short call is sold to offset the long put premium), and the probability of hitting the protection floor over the chosen tenor. Uses the user-supplied annualized implied volatility (sigma) when provided; otherwise falls back to a sector-typical IV. Pure deterministic computation: closed-form Black-Scholes; no network access; sector vol table and risk-free-rate curve are compiled in; identical inputs always return identical output to the cent. Returns a top-level object with keys: `inputs` (echoed canonical input), `riskFreeRate` (used in Black-Scholes), `realWorldDrift` (from expectedReturn), `barePut` (strike, premium, annualCost, annualCostPct, maxLoss, badYearPrice, badYearDropPct, coveredLossAtBadYear, premiumToCoveredRatio, expectedProfit, premiumToExpectedProfitRatio), `collar` (putStrike, callStrike, netPremium, annualCost, annualCostPct, maxLoss, upsideCap, upsideCapPct, isZeroCost, capProbability), `payoffTable`, `payoffRange`, and `recommended` (the better of bare put vs collar given the inputs). Both `barePut` and `collar` blocks are always returned regardless of caller preference; the caller picks.' + STRICT_INPUT_NOTE_NO_TICKER,
    inputSchema: {
      type: 'object',
