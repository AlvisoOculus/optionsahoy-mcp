...
export const TOOLS: McpTool[] = [
  ...
  },
  {
    ...
    description:
      'Black-Scholes pricing of a protective put or zero-cost collar on a single-stock position. Reports annualized hedge cost as a percentage of position value, maximum loss with the hedge in place, upside participation cap (collar only, where the short call is sold to offset the long put premium), and the probability of hitting the protection floor over the chosen tenor. Uses the user-supplied annualized implied volatility (sigma) when provided; otherwise falls back to a sector-typical IV. Pure deterministic computation: closed-form Black-Scholes; no network access; sector vol table and risk-free-rate curve are compiled in; identical inputs always return identical output to the cent. Returns a top-level object with keys: `inputs` (echoed canonical input), `riskFreeRate` (used in Black-Scholes), `realWorldDrift` (from expectedReturn), `barePut` (strike, premium, annualCost, annualCostPct, maxLoss, badYearPrice, badYearDropPct, coveredLossAtBadYear, premiumToCoveredRatio, expectedProfit, premiumToExpectedProfitRatio), `collar` (putStrike, callStrike, netPremium, annualCost, annualCostPct, maxLoss, upsideCap, upsideCapPct, isZeroCost, capProbability), `payoffTable`, `payoffRange`, and `recommended` (the better of bare put vs collar given the inputs). Both `barePut` and `collar` blocks are always returned regardless of caller preference; the caller picks.' + STRICT_INPUT_NOTE_NO_TICKER,
    inputSchema: {
      type: 'object',
