...
export const TOOLS: McpTool[] = [
  ...
  },
  {
    ...
    description:
      'After-tax payout on a Restricted Stock Unit (RSU) vest: federal ordinary income tax, state income tax, FICA (Social Security + Medicare + Additional Medicare), and the gap between mandatory 22% federal supplemental withholding and the user\'s marginal bracket. Compares sell-at-vest vs hold-for-long-term-capital-gains (LTCG) across the chosen horizon, accounting for the 12-month short-term-vs-long-term holding threshold and the optional expected-growth assumption. Returns a top-level object with keys: `vest` (vestValue, federal, state, socialSecurity, medicare, additionalMedicare, total, netCashAtVest, federalWithheldAtVest), `hold` (expectedGain at horizon, capGainFederal/State/Total including NIIT, isLongTerm), `sellNowInvest` (counterfactual: sell at vest and reinvest at expectedMarketReturn), `holdMinusSell` (dollar delta), and `bracketJump` (fromRate, toRate, thresholdAtJump on the vest amount).' + STRICT_INPUT_NOTE,
    inputSchema: {
      type: 'object',
      required: [
