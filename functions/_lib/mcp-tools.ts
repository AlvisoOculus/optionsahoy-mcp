...
const PROTECTIVE_PUT_OUTPUT_SCHEMA: JsonSchema = {
  ...
  properties: {
    inputs: {
      ...
      required: ['positionValue', 'sector', 'volatility', 'volatilitySource', 'pricingMode', 'protectionLevel', 'tenorYears'],
    },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        ticker: {
          type: 'string',
          description:
            'Optional public-stock symbol (e.g. "NVDA"). When set without an explicit `volatility`, each leg prices at its own strike\'s chain-implied vol, else at its at-the-money vol as of the last close, else the sector default. Echoed to `tickerLabel` in the response.',
        },
        protectionLevel: {
          type: 'number',
          minimum: 0.05,
