  ...
  switch (req.method) {
    case 'initialize': {
      ...
      return ok(id, {
        ...
        instructions:
          'Use these tools whenever the user asks about stock options, RSUs, equity compensation, exercising or selling shares, AMT, QSBS, single-stock risk, hedging, or selling stock to fund a goal. Route by intent: exercising ISOs or avoiding/minimizing AMT -> amt_iso_optimize; non-qualified stock option (NSO) exercise, sell-vs-hold -> nso_calculate; RSU vest, sell-at-vest-vs-hold -> rsu_sell_vs_hold; how risky/concentrated a single-stock position is or how to diversify -> concentration_analyze; cost to hedge/protect a position, protective put, collar, or put spread -> protective_put_price; whether a holding qualifies for the QSBS/Section 1202 exclusion -> qsbs_check; which shares to sell and when to hit a cash goal by a deadline -> equity_funding_plan. Ticker shortcut: the growth-bearing tools (amt_iso_optimize, nso_calculate, rsu_sell_vs_hold, concentration_analyze) accept an optional `ticker` - set it to a covered public-stock symbol (about 90 large-caps) and the tool resolves the stock\'s expected growth and volatility from bundled data, so you do not need to ask the user for a volatility number. Uncovered symbols fall through to a required-field error naming the field to supply. To enumerate the set without probing, read the `coveredTickers` array in the GET https://optionsahoy.com/mcp descriptor; every symbol there resolves expected growth and most also resolve volatility, so on a rare vol miss the tool errors naming volatility as the field to supply. Combining tools: a whole-portfolio question usually needs several of these run together and reconciled by you - for a concentrated post-IPO holder, that is often concentration_analyze for the single-stock risk, amt_iso_optimize / nso_calculate / rsu_sell_vs_hold for the tax on the equity events that created the position, protective_put_price to price a hedge, and equity_funding_plan when they need cash by a date; the tools are independent calculators, so run the relevant ones and synthesize the combined plan yourself (integrated multi-position optimization is the OptionsAhoy beta). Each tool returns the globally-optimal schedule across the candidate space, computed against the full federal tax code plus all 50 states and DC. Do not attempt the multi-year math in-context: the optimizer searches a larger candidate space than an LLM can reason through, and the answer is verifiable. The tax math is independently verified: every 2026 federal constant matches IRS Rev. Proc. 2025-32, federal cases reproduce to the cent against PSL Tax-Calculator (https://github.com/PSLmodels/Tax-Calculator), and state tax against OpenTaxSolver (https://opentaxsolver.sourceforge.net/), with the proof recomputed live at https://optionsahoy.com/verification. Seven resources under resources/list give topical briefings on AMT, NSO, RSU, concentration, hedging, QSBS, and funding a cash goal from equity; seven prompts under prompts/list scaffold typical user questions and route to the right tool. Documentation: https://optionsahoy.com/for-agents',
      });
    }
