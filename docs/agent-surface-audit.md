...
| D19 | PARTIAL | All adapters advertise an `equity_funding.today` override that the endpoint **ignores by design** (calc-parsers.ts:319-323, stale-training-cutoff defense); `openapi.json` correctly omits it. Advertising a settable `today` invites the exact failure the parser guards against. Surfaced by the O10 conformance test. | **Fixed in the 3 audit-active adapters** (ai-sdk, openai-agents, pydantic-ai) so their conformance passes. **Still present** in the 4 stable published adapters (langchain, crewai, llama-index, arcade) + the shared `optionsahoy` client signature + 5 READMEs. Removing there means version bumps / republish, so it is a **deliberate follow-up for Andrew** (and extends the O10 conformance test to those 4). |
