...
| **D3** | DEFERRED | `concentration_analyze` validates a full `hedgeChoice` object the MCP handler never reads; always returns a 30%-OTM 1yr *put* regardless of the collar/tenor passed | handler -> `calculate()` (`mcp-tools.ts:1210`); `hedgeChoice` only in `buildCustomPlan` (`concentration.ts:772-777`) | Fork needs a decision: **remove** cascades to ~10 files incl. every integration + drops a documented capability; **thread** wires `buildCustomPlan` into the handler and delivers it. Own PR. |
