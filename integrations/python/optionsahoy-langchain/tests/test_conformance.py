...
"""Conformance guard: each tool's pydantic args_schema must expose exactly the
...
def test_every_tool_schema_matches_openapi():
    ...
    for name, slug in NAME_TO_SLUG.items():
        ...
        props = set(tools[name].args_schema.model_json_schema().get("properties", {}).keys())
        assert props == by_slug[slug], (
            f"{name} args_schema diverged from openapi /api/v1/{slug}: "
