"""Tests for the LangChain OptionsAhoy tools.

The OptionsAhoy client is mocked so no network call is made; tests assert tool
shape (name, description, args_schema) and that invoking a tool routes through the
client and returns the parsed dict.
"""
...
def test_each_tool_has_name_description_args_schema():
    for tool in get_optionsahoy_tools():
        ...
        # args_schema exposes its fields
...
def test_invoking_tool_routes_through_client_and_returns_dict():
    ...
    out = tools["optionsahoy_qsbs_check"].invoke(
        {
...
def test_nso_tool_routes_to_nso_method():
    ...
    tools = {t.name: t for t in get_optionsahoy_tools(client=fake)}
    out = tools["optionsahoy_nso_calculate"].invoke(
        {
            "shares": 1000,
            "strike": 2.0,
            "currentPrice": 20.0,
...
def test_invalid_args_raise_validation_error():
    ...
    with pytest.raises(Exception):
        ...
        tools["optionsahoy_qsbs_check"].invoke({"acquisitionDate": "2018-01-01"})
