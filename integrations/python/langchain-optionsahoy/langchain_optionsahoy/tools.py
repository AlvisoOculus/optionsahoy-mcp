"""LangChain ``StructuredTool`` wrappers for the OptionsAhoy calculators.

One tool per public endpoint. Each tool's ``args_schema`` is a pydantic v2 model
mirroring the endpoint's required and optional fields exactly as published in the
OpenAPI schema. Tool descriptions say what each tool computes, not how, and
deabbreviate acronyms on first use.
"""
...
def get_optionsahoy_tools(client: Optional[OptionsAhoyClient] = None) -> List[StructuredTool]:
    """Return one LangChain StructuredTool per OptionsAhoy calculator endpoint.
    ...
    Returns:
        A list of StructuredTools, each with a name, description, and pydantic
        args_schema mirroring the underlying endpoint.
    """
    oa = client or OptionsAhoyClient()
    tools: List[StructuredTool] = []
    for name, method_name, args_schema, description in _SPECS:
        method = getattr(oa, method_name)
